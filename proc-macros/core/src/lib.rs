...
pub fn context(item: TokenStream) -> Result<TokenStream, Error> {
    ...
    match &ast.data {
        ...
        _ => {
            return Err(Error::new_spanned(
                ast,
                format!("{pkg_name}::Context can only be derived on struct types"),
            ))
        }
    };
    ...
    let decision_maker_field_types = matched_decision_maker_attributes
        .iter()
        .flat_map(|MatchedAttribute { field, .. }| matched_storage_client_attributes.iter().map(move |_| &field.ty))
        .collect::<Vec<_>>();
    ...
    let decision_maker_field_accessors = matched_decision_maker_attributes
        .iter()
        .flat_map(|MatchedAttribute { field_accessor, .. }| {
            matched_storage_client_attributes.iter().map(move |_| field_accessor)
        })
        .collect::<Vec<_>>();
    ...
    let tokens = quote! {
        #(
            impl #impl_generics authzen::Context<#decision_maker_field_types, #storage_client_field_types> for #ident #ty_generics #where_clause {
                type Context<#gat_lifetime> = #context_field_gat;
                type Subject<#gat_lifetime> = #subject_field_gat;
