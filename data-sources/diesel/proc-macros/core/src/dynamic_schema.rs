    ...
    fn to_tokens(&self, token_stream: &mut TokenStream) {
        ...
        let joinable_tokens = joinables
            ...
            .map(|joinable| {
                ...
                quote!((
                    authzen_diesel::JoinablePair { child_table_name: #child_table, parent_table_name: #parent_table },
                    #joinable,
                ))
            })
            .collect::<Vec<_>>();
...
impl ToTokens for Column {
    fn to_tokens(&self, token_stream: &mut TokenStream) {
        ...
        let sql_type = if is_nullable {
            quote! { authzen_diesel::ColumnSqlType::IsNullable(std::sync::Arc::new(Box::new(Nullable::<#ty>::default()))) }
        } else {
            quote! { authzen_diesel::ColumnSqlType::NotNull(std::sync::Arc::new(Box::new(#ty::default()))) }
        };
        token_stream.append_all(quote! {
            authzen_diesel::Column {
                name: #name,
                sql_type: #sql_type,
            }
        });
...
pub fn dynamic_schema(ident: syn::Ident, tokens: TokenStream) -> Result<TokenStream, Error> {
    ...
    let ident_snake = format_ident!("authzen_diesel_{}", format!("{ident}").to_case(Case::Snake));
    ...
    let tokens = quote! {
        pub mod #ident_snake {
            ...
            #[derive(Clone, Debug)]
            pub struct #ident_pascal(pub authzen_diesel::DynamicSchema);

            impl std::ops::Deref for #ident_pascal {
                type Target = authzen_diesel::DynamicSchema;
                fn deref(&self) -> &Self::Target {
                    &self.0
                }
            }

            authzen_diesel::lazy_static::lazy_static! {
