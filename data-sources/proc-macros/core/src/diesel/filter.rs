...
fn get_dynamic_query(
    ...
    NestedQuery {
        multipaginated: quote! {
            #tokens
            ::authzen::data_sources::proc_macros_core::diesel::reexports::diesel::_Db::query(&db, move |conn| Box::pin(query
                #group_by_clause
                #order_by_clauses
...
fn get_static_query(
    ...
    if filter_index == filters.len() {
        ...
        return NestedQuery {
            multipaginated: quote! {
                ::authzen::data_sources::proc_macros_core::diesel::reexports::diesel::_Db::query(&db, move |conn| Box::pin(#base_query
                    #(#filter_clauses)*
                    #group_by_clause
                    #order_by_clauses
                    #select_clause
